#!/usr/bin/env python3
"""Bridge existing EO ROS2 YOLO topics to GUI UDP packets."""
...
IMAGE_TOPIC = os.getenv("IMAGE_TOPIC", "/yolo/eo/image_raw")
...
SOURCE_NAME = os.getenv("SOURCE_NAME", "eo")
...
def encode_frame_for_udp(frame: np.ndarray) -> tuple[bytes, int, int] | None:
    ...
    for scale in scale_attempts:
        ...
        for quality in quality_attempts:
            ...
            if ok and len(encoded) <= max_image_payload_bytes:
                return encoded.tobytes(), int(working_frame.shape[1]), int(working_frame.shape[0])
...
class GuiCameraBridge(Node):
    def __init__(self) -> None:
        ...
        self.get_logger().info(f"EO image topic: {IMAGE_TOPIC}")
        self.get_logger().info(f"EO detection topic: {DETECTION_TOPIC}")
        self.get_logger().info(f"EO status topic: {STATUS_TOPIC}")
        self.get_logger().info(f"Streaming EO GUI packets to {GUI_HOST}:{GUI_PORT}")
    ...
    def _on_image(self, message: Image) -> None:
        try:
            ...
            encoded_result = encode_frame_for_udp(frame)
            if encoded_result is None:
                self.get_logger().warning("EO frame was too large to encode into one UDP packet.")
                return

            encoded, stream_width, stream_height = encoded_result
    ...
    def _on_detections(self, message: Detection2DArray) -> None:
        ...
        try:
            ...
            self.get_logger().warning(f"Failed to forward EO detections: {exc}")
    ...
    def _on_status(self, message: YoloStatus) -> None:
        ...
        try:
            ...
            self.get_logger().warning(f"Failed to forward EO status: {exc}")
