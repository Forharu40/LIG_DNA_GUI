...
현재 범위는 IR만 대상으로 합니다. 운용통제 노트북의 실제 UDP 수신 포트가 `5000`이므로, IR 브리지는 `5000` 포트로 GUI에 전달합니다.
...
모든 패킷은 기본적으로 `GUI_HOST:5000`로 전송합니다.
...
운용통제 PC에서는 GUI를 실행하면 됩니다. 현재 `ver4` 브랜치는 IR 실험 기준으로 IR `5000` 포트를 사용합니다.
